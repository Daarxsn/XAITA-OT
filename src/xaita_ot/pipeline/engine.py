...
from ..core.context import contextualize
...
class XAITAEngine:
    def __init__(self, config: AppConfig):
        self.config=config; set_seed(config.seed)

    def analyze_events(self, events, hypotheses=None):
        episodes=reconstruct(events,self.config.correlation.threshold,self.config.correlation.temporal_window_seconds)
        outputs=[]
        hypotheses=hypotheses or ['H1','H2','H3']
        for ep in episodes:
            ctx=contextualize(ep)
            ...
            mas=0.50+0.50*(len(set(e.asset for e in ep.events))>0)
            base={'DC':dc,'BSS':bss,'ECS':ecs,'EC':ec,'MAS':mas}
            evidence={h:{k:max(0,min(1,v*(1-0.08*i))) for k,v in base.items()} for i,h in enumerate(hypotheses)}
            attrs=assess(hypotheses,evidence,self.config.attribution.reliability)
            best=to_dict(attrs[0]) if attrs else None
            risk=score_risk(severity=max(0.0,dc),operational_impact=min(1,0.4+0.1*len(ep.events)),criticality=0.7,attribution_evidence=best['belief'] if best else 0.0,weights=self.config.risk.weights)
            fi=feature_importance_linearized(np.array([[list(e.features.values()) for e in ep.events]],dtype=np.float32) if ep.events and ep.events[0].features else np.zeros((1,1,1)), list(ep.events[0].features.keys()) if ep.events and ep.events[0].features else ['unknown'])
