...
def run_detection(csv_path: str | Path, dataset: str, config: AppConfig, seed: int | None = None) -> ExperimentRun:
    ...
    return ExperimentRun(
        ...
        dataset=dataset, seed=run_seed, started_at=datetime.now(timezone.utc).isoformat(),
...
def aggregate_runs(runs: list[ExperimentRun]) -> dict:
    if not runs: return {'runs': [], 'aggregate': {}}
    metric_values = {}
    for run in runs:
    ...
    aggregate = {model: {metric: {'mean': float(np.mean(values)), 'std': float(np.std(values, ddof=1)) if len(values) > 1 else 0.0, 'n': len(values)} for metric, values in metrics.items()} for model, metrics in metric_values.items()}
