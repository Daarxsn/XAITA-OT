...
def train_baselines(train_wd, val_wd, test_wd, cfg, seed=None):
    """Train RF, CNN, LSTM and CNN-LSTM on identical leakage-controlled windows."""
    if seed is not None:
        set_seed(seed)
    out = {}
    flat_tr = train_wd.X.reshape(len(train_wd.X), -1)
    flat_te = test_wd.X.reshape(len(test_wd.X), -1)
    rf = RandomForestClassifier(n_estimators=120, random_state=cfg.seed if seed is None else seed, n_jobs=-1, class_weight='balanced')
    rf.fit(flat_tr, train_wd.y)
    out['random_forest'] = binary_metrics(test_wd.y, rf.predict_proba(flat_te)[:, 1])
