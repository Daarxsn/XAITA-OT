...
def shap_feature_importance(model, background, samples, feature_names, top_k=8):
    ...
    Background and samples must be NumPy arrays shaped [N, T, F]. The function
    intentionally fails loudly if SHAP cannot explain the supplied model rather
    than silently substituting a heuristic explanation.
    """
    try:
        import shap
        import torch
    ...
    return [
        {
            ...
            "importance": float(importance[i] / total),
